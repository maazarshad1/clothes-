        ...
        >
          ...
          <div className="hidden md:flex flex-col gap-4 w-20">
            {images.map((img, idx) => (
              <button
                ...
                className={`aspect-[3/4] border-2 transition-all overflow-hidden ${activeImageIndex === idx ? 'border-black' : 'border-transparent opacity-60 hover:opacity-100'}`}
              >
            ...
            {product.video && (
              <button
                ...
                className="aspect-[3/4] bg-gray-100 flex items-center justify-center border-2 border-transparent hover:border-black transition-all relative overflow-hidden group"
              >
                <img src={images[0]} alt="Video thumbnail" className="w-full h-full object-cover opacity-50" />
                <Play size={24} className="absolute z-10 text-black group-hover:scale-110 transition-transform" />
              </button>
            )}
          </div>
          ...
          <div className="flex-1 aspect-[3/4] bg-gray-100 overflow-hidden relative group">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 md:hidden">
              {images.map((_, idx) => (
                <button
                  key={idx}
                  onClick={() => setActiveImageIndex(idx)}
                  className={`w-2 h-2 rounded-full transition-all ${activeImageIndex === idx ? 'bg-black w-4' : 'bg-black/20'}`}
                />
              ))}
            </div>
            ...
            {product.video && (
              <button
                onClick={() => setIsVideoOpen(true)}
                className="absolute top-4 right-4 md:hidden w-10 h-10 bg-white/80 backdrop-blur-sm flex items-center justify-center rounded-full"
              >
                <Play size={20} />
              </button>
            )}
        ...
        >
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 mt-8">
              <div className="border border-gray-100 p-4 rounded-sm text-center">
                <p className="text-[10px] uppercase tracking-widest text-gray-400 mb-1">Shipping</p>
                <p className="text-xs font-semibold">Free Express</p>
              </div>
              <div className="border border-gray-100 p-4 rounded-sm text-center">
                <p className="text-[10px] uppercase tracking-widest text-gray-400 mb-1">Returns</p>
                <p className="text-xs font-semibold">30-Day Window</p>
              </div>
            </div>
          </div>
        </motion.div>
