    ...
    <div className="min-h-screen bg-black text-white pt-20">
      ...
      <div className="bg-black">
        {sections.map((section, sIdx) => {
          ...
          return (
            <section key={section.id || section.category} className="py-12 md:py-24 border-b border-white/5 relative">
              <div className="container mx-auto px-4 relative z-0">
                <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-10 md:mb-16 border-b border-white/10 pb-6 md:pb-8 gap-6">
                  ...
                  <Link to={linkPath} className="text-white/40 uppercase tracking-[0.4em] text-[10px] md:text-[11px] font-black flex items-center gap-2 hover:text-[#C2A46C] transition-all group self-start md:self-auto py-4 px-6 -mr-6 -mb-4 bg-transparent z-20">
                    VIEW ALL <ArrowRight size={14} className="group-hover:translate-x-2 transition-transform" />
                  </Link>
                </div>
                
                <div className="grid grid-cols-2 gap-8 md:gap-16 lg:gap-24 max-w-6xl mx-auto">
                  {sectionProducts.map((product, index) => (
                    <div 
                      key={product.id}
                      className="group flex flex-col relative"
                    >
                      <Link to={`/product/${product.id}`} className="block w-full mb-8 relative z-10">
                        <div className="relative aspect-[4/5] overflow-hidden bg-white/5 shadow-2xl group-hover:shadow-[0_30px_60px_rgba(0,0,0,0.8)] transition-all duration-700">
                          <img 
                      ...
                      <div className="space-y-3 relative z-10">
                        <Link to={`/product/${product.id}`} className="block group/title">
                          <h3 className="font-serif text-xl tracking-tight text-white/90 group-hover/title:text-[#C2A46C] transition-colors line-clamp-2 uppercase leading-snug">
                            {product.name}
                          </h3>
                        </Link>
